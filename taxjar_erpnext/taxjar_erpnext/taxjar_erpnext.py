...
def get_taxjar_account(company):
	...
	account_name = frappe.db.get_value(
		"TaxJar Account", {"company": company, "taxjar_calculate_tax": 1}
	)
...
def get_client(company):
	...
	if taxjar_account.is_sandbox:
		api_key = (
			taxjar_account.get_password("sandbox_api_key") if taxjar_account.sandbox_api_key else None
		)
...
def get_state_code(address, location):
	...
	if address is not None:
		...
		if state_code not in SUPPORTED_STATE_CODES:
			frappe.throw(
				_(
					"The state '{0}' in {1} Address {2} is not a supported US state.<br><br>"
					"Please enter a valid 2-letter US state code."
				).format(address.get("state"), location, address_link),
...
def set_sales_tax(doc, method):
	...
	is_non_nexus_quote = (
		doc.doctype == "Quotation" and not has_nexus and taxjar_account.calculate_tax_for_all_states
	)
...
def check_sales_tax_exemption(doc, taxjar_account):
	...
	if not sales_tax_exempted and customer_name:
		if frappe.db.has_column("Customer", "exempt_from_sales_tax"):
			sales_tax_exempted = frappe.db.get_value("Customer", customer_name, "exempt_from_sales_tax")
...
def get_company_address_details(doc):
	...
	if not company_address_data or not company_address_data.company_address:
		...
		frappe.throw(
			_(
				"No default address found for company {0}.<br><br>"
				"Please set a default company address in the Company settings."
			).format(company_link),
...
def get_iso_3166_2_state_code(address):
	...
	if not state:
		frappe.throw(
			_(
				"State is required in address {0}.<br><br>"
				"Please open the address and add a valid US state code (e.g., TX, CA, FL)."
			).format(address_link),
